...
class GameScene: SKScene {
    ...
    var team: Team = Team.ND
    
    override func didMove(to view: SKView) {
        
        DataHandeler.instance.GetActiveGames { (arr, _) in
            let currentGameData = arr[self.ID] as! [String: Any]

            DataHandeler.instance.GetUsername(email: currentGameData["email"] as! String) { (username) in
            ...
            if currentGameData["team"] == nil {
                self.team = .Square
            } else {
                self.team = .Circle
            }

            if currentGameData["turn"] != nil {
                self.turn = currentGameData["turn"] as! Int
            }
                
            if let positions = currentGameData["positions"] as? NSArray {
                for i in 0...10 {
                    if i % 2 == 0 {
    ...
    func UpdateTurn(amount: Int) {
        let myData = DataHandeler.instance._REF_USERS_.child(Auth.auth().currentUser!.uid).child("active_games").child(String(self.ID))
        
        myData.observeSingleEvent(of: .value) { (snapshot) in
            self.turn += amount
            
            let temp = snapshot.value as! [String: Any]
            myData.updateChildValues(["turn": self.turn])
